...
# Google Apps Script – تحديثات ضرورية لمنع تكرار الهوية ولجلب البيانات إلى لوحة الإدارة

أضِف/حدّث هذا الكود في مشروع Google Apps Script المرتبط بملف:
- اسم الملف: دورة_الصيانة_المنزلية
- الورقة: Sheet1
- المعرّف: 1R2ebQsZnuc_-hGZAtZKEmNmI23jic2_LeDGXV4KCVN4

## منع التكرار + الحفظ (doPost) وواجهات قراءة (doGet)
function doPost(e) {
  var sheet = SpreadsheetApp.openById("1R2ebQsZnuc_-hGZAtZKEmNmI23jic2_LeDGXV4KCVN4").getSheetByName("Sheet1");
  var data = JSON.parse(e.postData.contents || "{}");
  // منع التكرار حسب رقم_الهوية (العمود B)
  var idsRange = sheet.getRange(2, 2, Math.max(sheet.getLastRow()-1,0), 1).getValues();
  var exists = idsRange.some(function(r){ return String(r[0]) === String(data.id); });
  if (exists) {
